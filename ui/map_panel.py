...
def render_map_section():
    ...
    st.markdown("<div class='astro-card' style='padding:0; background:var(--color-card); border:1px solid var(--color-border);'>", unsafe_allow_html=True)
    ...
    st.markdown(
        """
        <style>
        .map-data-card {background:var(--color-card); border:1px solid var(--color-border); border-radius:14px; padding:0.85em 1.05em; margin-top:0.8em; box-shadow:var(--shadow-sm);}
        .map-badge {display:inline-block; background:var(--color-card-accent); color:var(--color-text); font-size:0.6rem; letter-spacing:0.5px; padding:3px 8px; border-radius:10px; font-weight:600; margin-right:6px;}
        .map-badge.owm {background:#334155; color:#f1f5f9;}
        .map-field {font-size:0.8rem; margin:2px 0; color:var(--color-text);}
        </style>
        """,
